import PropTypes from 'prop-types'

/**
 * @info ProjectAdvancedForm (05 Mar 2021) // CREATION DATE
 *
 * @comment ProjectAdvancedForm - React component.
 *
 * @since 05 Mar 2021 ( v.0.0.1 ) // LAST-EDIT DATE
 *
 * @return {ReactComponent}
 */

const ProjectAdvancedForm = (props) => {
  // [INTERFACES]
  /*
  code sample: 
  const { data } = props
  */

  // [ADDITIONAL_HOOKS]
  /*
  code sample: 
  const firestore = useFirestore()
  */

  // [COMPONENT_STATE_HOOKS]
  /*
  code sample:
  const singleton = useRef(true) // references also put here
  const [state, setState] = useState({})
  */

  // [HELPER_FUNCTIONS]

  // [COMPUTED_PROPERTIES]
  /* 
    code sample: 
    const userDisplayName = user.firstName + user.lastName
  */

  // [USE_EFFECTS]

  // [TEMPLATE]
  return <div>ProjectAdvancedForm</div>
}

// [PROPTYPES]
ProjectAdvancedForm.propTypes = {}

export default ProjectAdvancedForm
